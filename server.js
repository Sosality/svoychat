...
// 🔒 Шифрование приватного ключа для хранения
function encryptPrivateKey(privKey, password) {
  // Можно использовать пароль пользователя или SECRET_KEY сервера
...
app.post("/api/login", async (req, res) => {
  ...
  try {
    ...
    // Расшифровываем приватный ключ на клиенте (или по паролю)
    ...
    res.json({
      ...
      pubKey: user.pub_key,
      privKey: privDecrypted, // ⚠️ Отправлять только если это доверенный HTTPS, иначе можно убрать
    });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: "Internal error" });
...
app.get("/api/keys/:username", async (req, res) => {
  try {
    const { rows } = await pool.query("SELECT pub_key FROM users WHERE username=$1", [
